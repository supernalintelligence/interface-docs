...
export default function StoriesPage() {
  ...
  const handleExecuteStory = async (featureName: string) => {
    ...
    try {
      ...
      addMessage(
        `✅ Story "${featureName}" passed all ${mockResults.length} scenarios in ${totalDuration}ms`,
        'system'
      );
    } catch (error) {
      addMessage(`❌ Error executing story: ${(error as Error).message}`, 'system');
      setFeatures(prev => prev.map(f => 
              ...
              >
                <div className="p-6">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => router.push(`/stories/${feature.path}`)}
